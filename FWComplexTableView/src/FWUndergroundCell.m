...
- (instancetype)initWithStyle:(UITableViewCellStyle)style
  ...
  if (self) {
    ...
    _pullDownView = [[FWPullDownView alloc]
        initWithFrame:CGRectMake(0, -30, self.frame.size.width, 30)];
...
- (void)setScrollView:(UIScrollView *)scrollView {
  ...
  [_pullDownView
      setFrame:CGRectMake(0, -200, _scrollView.frame.size.width, 200)];
  ...
  [_scrollView.panGestureRecognizer
      addTarget:self
         action:@selector(gestureRecognizerUpdate:)];
}

- (void)gestureRecognizerUpdate:(UIPanGestureRecognizer *)panGes {
